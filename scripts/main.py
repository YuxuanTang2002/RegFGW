...
def main():
    p = argparse.ArgumentParser("regfgw main")

    # Inputs
    p.add_argument("--substrate", required=True, help="substrate bulk cif")
    p.add_argument("--film", required=True, help="film bulk cif")
    ...
    # Output switches (separated responsibilities)
    ...
    records = builder.sum_interface_records(
    ...
    # Encoder + FGW + BO
    # -----------------------------
    enc = GraphEncoder(embedding_path=args.embedding)

    fgw_builder = FGWBuilder(FGWBuildParams())
    scorer = FGWScorer(
        builder=fgw_builder,
    ...
    bo = RegistryPriorBO(
        ...
        structure_check=args.structure_check,  # only controls intermediate dumps
    )

    # -----------------------------
    # Run BO per interface candidate
    # -----------------------------
    for interface in tqdm(records, desc="Computing interface registries"):
        ...
        print(
            ...
            f"best_score={best_record.score:.6f}"
        )
